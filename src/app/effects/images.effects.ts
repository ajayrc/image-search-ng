...
export class ImageEffects {
  ...
  getImages$ = createEffect(() => this.actions$.pipe(
    ofType(imagesGetAction),
    switchMap(action => { // can use concatMap/mergeMap to allow multiple api calls - less performant
      loaderStopAction(); // todo not working!!!
      return this.imageService.searchImage(action.userInput)
        .pipe(
          map(images => {
            return imagesReceivedAction({ imagesResult: images });
          }),
          catchError(() => EMPTY)
        )
    })
